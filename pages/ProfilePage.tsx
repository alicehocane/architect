...
const ProfilePage: React.FC<ProfilePageProps> = ({ architect, onBackClick, onArchitectClick }) => {
  ...
  const profileFaqs = [
    {
      ...
      answer: `AAK Architects is highly rated in ${primaryCity}. They are known for smart designs that save energy and look great. They work on both houses and commercial buildings.`
    },
    {
    ...
    },
    {
      ...
      answer: `Fees depend on the project size. Most top firms in ${primaryCity} charge a percentage of the total construction cost (usually 3% to 7%) or a fixed rate per square foot.`
    },
    {
      question: `Does ${architect["Shop Name"]} also build houses?`,
      answer: `Many design firms also handle construction. We recommend asking the ${architect["Shop Name"]} team directly if they offer full "Design-Build" services.`
    }
  ];

  useEffect(() => {
    // 1. Update Title
    document.title = `${architect["Shop Name"]} | Best Architects in ${primaryCity}`;
    ...
    metaDescription.setAttribute('content', `Contact ${architect["Shop Name"]} in ${primaryCity}. See their phone number, office location, and ratings. Find top architects on Architectorly.`);
  ...
  useEffect(() => {
    ...
    const locationSchemas = architect.Locations.map(loc => ({
      ...
      "@type": "LocalBusiness",
      "@id": `https://www.architectorly.com/architects/${architect.slug}/${loc.citySlug}`,
      "name": `${architect["Shop Name"]} - ${loc.City} Branch`,
      "image": `https://picsum.photos/seed/${architect.slug}/600/600`,
      "telephone": loc["Phone Number"],
      "url": architect.Website || `https://www.architectorly.com/architects/${architect.slug}`,
      "hasMap": loc["Map URL"],
      "priceRange": "$$$",
      "address": { 
        "@type": "PostalAddress", 
        "streetAddress": loc.Address || "Address available on request", 
        "addressLocality": loc.City, 
        "addressCountry": "PK" 
      },
      "aggregateRating": loc.Rating ? { "@type": "AggregateRating", "ratingValue": loc.Rating, "reviewCount": loc.Reviews || 1 } : undefined,
      "parentOrganization": { "@type": "Organization", "name": architect["Shop Name"], "url": architect.Website || "https://www.architectorly.com" }
    }));
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "Architects", "item": "https://www.architectorly.com/top-rated" },
    ...
    <div className="max-w-[1024px] mx-auto px-6 py-12 pb-32 page-transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 border-t border-[#d2d2d7]/30 pt-20">
        <div className="lg:col-span-2 space-y-12">
          <div>
            <h3 className="text-[28px] font-bold text-[#1d1d1f] mb-6">About This Firm</h3>
            <p className="text-[21px] text-[#424245] leading-relaxed font-light">
              {architect["Shop Name"]} is a professional design firm operating in {architect.Locations.map(l => l.City).join(', ')}. They focus on modern design standards and helping clients build better spaces. Their team works to ensure quality in every project they undertake.
            </p>
          </div>
          <div className="p-12 bg-[#f5f5f7] rounded-[3rem] border border-[#e5e5e7] flex flex-col md:flex-row items-center gap-8 shadow-sm">
             <div className="flex-1">
               <h4 className="text-[26px] font-bold text-[#1d1d1f] mb-2 tracking-tight">Need a Cost Estimate?</h4>
               <p className="text-[#86868b] font-light text-[18px]">Use our free calculator to see how much it costs to build a house in Pakistan right now.</p>
             </div>
             <a href="/estimate-calculator" className="bg-[#0071e3] text-white px-10 py-5 rounded-2xl font-bold hover:bg-[#0077ed] transition-all shadow-xl shadow-blue-500/10 active:scale-95 whitespace-nowrap text-[18px]">
               Calculate Cost
             </a>
          </div>
        </div>
        <div className="space-y-10">
           <h3 className="text-[24px] font-bold text-[#1d1d1f]">Why Choose Them?</h3>
           <ul className="space-y-6">
              {[
                { title: 'Verified', desc: 'Listed on Architectorly with checked details.' }, 
                { title: 'Experienced', desc: 'Proven track record in the local market.' }, 
                { title: 'Accessible', desc: 'Active contact numbers and office locations.' }
              ].map((item, i) => (
