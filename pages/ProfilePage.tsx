...
const ProfilePage: React.FC<ProfilePageProps> = ({ architect, onBackClick, onArchitectClick }) => {
  useEffect(() => {
    ...
    const locationSchemas = architect.Locations.map(loc => ({
      ...
      "hasMap": loc["Map URL"],
      "priceRange": "$$$",
      "address": { "@type": "PostalAddress", "addressLocality": loc.City, "addressCountry": "PK" },
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-20 py-12 pb-32 page-transition">
      ...
      <section className="mb-24">
        ...
        <div className={`grid gap-8 ${hasMultipleLocations ? 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-[17px] text-[#86868b] font-medium mb-4">{loc["Phone Number"] || 'Direct Line Pending'}</p>
                  </div>
                  {loc.Rating && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 border-t border-[#d2d2d7]/30 pt-20">
        <div className="lg:col-span-2 space-y-12">
          ...
          <div className="p-12 bg-[#f5f5f7] rounded-[3rem] border border-[#e5e5e7] flex flex-col md:flex-row items-center gap-8 shadow-sm">
             ...
             <a href="https://api.whatsapp.com/send/?phone=923215201830" target="_blank" rel="noopener noreferrer" className="bg-[#0071e3] text-white px-10 py-5 rounded-2xl font-bold hover:bg-[#0077ed] transition-all shadow-xl shadow-blue-500/10 active:scale-95 whitespace-nowrap text-[18px]">Free Estimate</a>
          </div>
        </div>
      ...
      {relatedArchitects.length > 0 && (
        <section className="mt-32 pt-20 border-t border-[#d2d2d7]/50">
          <div className="flex items-center justify-between mb-12">
            ...
            {relatedCitySlug && (
              <button onClick={() => window.location.hash = `city/${relatedCitySlug}`} className="hidden sm:flex items-center gap-2 text-[#0066cc] font-semibold hover:underline">
                View more in {relatedCity}
