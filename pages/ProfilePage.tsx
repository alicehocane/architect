...
const ProfilePage: React.FC<ProfilePageProps> = ({ architect, onBackClick, onArchitectClick }) => {
  ...
  const relatedArchitects = useMemo(() => {
    ...
    // 1. Always try to add AAK Architects first
    const aak = getArchitectBySlug('aak-architects');
    if (aak && aak.slug !== architect.slug) {
      finalRecommendations.push(aak);
    }

    // 2. Add local city professionals (excluding current profile and AAK)
    if (relatedCitySlug) {
      ...
      {relatedArchitects.length > 0 && (
        <section className="mt-32 pt-20 border-t border-[#d2d2d7]/50">
          <div className="flex items-center justify-between mb-12">
            ...
            {relatedCitySlug && (
              <button 
                onClick={() => window.location.hash = `city/${relatedCitySlug}`}
                className="hidden sm:flex items-center gap-2 text-[#0066cc] font-semibold hover:underline"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {relatedArchitects.map((related) => (
              <ArchitectCard 
                ...
                cityContextSlug={relatedCitySlug}
                isRecommended={related.slug === 'aak-architects'}
              />
            ))}
          </div>
          ...
          {relatedCitySlug && (
            <button 
              onClick={() => window.location.hash = `city/${relatedCitySlug}`}
              className="w-full mt-10 sm:hidden py-4 rounded-2xl bg-[#f5f5f7] text-[#1d1d1f] font-bold text-[15px]"
            >
              View more {relatedCity} Firms
            </button>
          )}
        </section>
      )}
